...
module.exports = {
   ...
   */
	find: (req, res) => {
		knex
			.select('markets.*', knex.raw('JSON_AGG(hours.*) AS hours'))
			.from('markets')
   ...
   */
	...
	/**
   * findByIdAndReplace() updates entire market from Markets table matching unique _id property.
   * @param {Object} req - Express request object.
   * @param {Object} res - Express response object.
   * @return {Market}
   */
	findByIdAndReplace: (req, res) => {
   ...
   */
	...
	addNewMarket: (req, res) => {
		const formData = req.body;
		knex('markets').insert(formData).then((data) => res.send(data)).catch((err) => console.error(err));
	}
};
